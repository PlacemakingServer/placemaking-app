...
export default function ResearchAnswers() {
  ...
  const contributorId = router.query.contributor_id;
  ...
  return (
    ...
    >
      ...
      {otherFields.map(field => {
        ...
        return (
          <Card key={field.id} className={field.required ? "border-l-4 border-blue-500" : ""}>
            <CardContent className="flex flex-col gap-2">
              {typeName !== "long_text" && typeName !== "Múltipla Escolha" && (
                <div className="relative">
                  ...
                  >
                    {field.title}{field.required && <span className="text-red-500 ml-1">*</span>}
                  </Label>
                </div>
              )}
              ...
              return (
                <Card key={field.id} className="p-4 flex flex-col items-center">
                  <CardContent className="flex flex-col items-center gap-2">
                    <span className="text-gray-700 font-medium text-center">{field.title}</span>
                    <div className="flex items-center gap-4 mt-2">
                      <motion.button
                       type="button"
                        whileTap={{ scale: 0.9 }}
                        onClick={() => handleChange(field.id, Math.max(val - 1, 0))}
